...
BUFFER_SIZE = 8 # TEXT streams
BATCH_STREAM_LENGTH = 1000
SETTING = '/scratch/cluster/vsub/lstmdirs/vidh5'
OUTPUT_DIR = '{0}/hdf5/buffer_{1}_yts2s_cocolm_{2}'.format(SETTING, BUFFER_SIZE, MAX_WORDS)
VOCAB = '%s/vocabulary.txt' % OUTPUT_DIR
...
OUT_FILE_PATTERN = \
'/scratch/cluster/vsub/youtube_dataset/lstmdirs/ytSequence/rawcorpus/{0}/yts2s_cocolm_{0}_sequence.txt'
OUT_CORPUS_PATH = '/scratch/cluster/vsub/youtube_dataset/lstmdirs/ytSequence/rawcorpus/{0}'
...
def preprocess_dataset(split_name, data_split_name, batch_stream_length,
  ...
  vid_id_order_outpath = '%s/yts2s_cocolm_%s_vidid_order_%d_%d.txt' % \
  (out_path, data_split_name, BUFFER_SIZE, MAX_WORDS)
  frame_sequence_outpath = '%s/yts2s_cocolm_%s_sequence_%d_%d_recurrent.txt' % \
  (out_path, data_split_name, BUFFER_SIZE, MAX_WORDS)
