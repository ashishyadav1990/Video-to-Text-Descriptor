...
from framefc7_stream_80kvocab_text_to_hdf5_data import *
...
def encode_video_frames(net, video_features, previous_word=-1):
  for frame_feature in video_features:
    ...
    inv_stage_ind = np.array([1]) # keep fc7
    ...
    net.forward(frames_fc7=image_features, cont_sentence=cont, input_sentence=data_en,
       stage_indicator=stage_ind, inv_stage_indicator=inv_stage_ind)
...
def predict_single_word(net, pad_img_feature, previous_word, output='probs'):
  ...
  inv_stage_ind = np.array([0]) # zero out fc7
  image_features = np.zeros_like(net.blobs['frames_fc7'].data)
...
# TODO: modify this as well. After the first word the subsequent words have no img
...
def main():
  LSTM_DIR = '/scratch/cluster/vsub/lstmdirs'
  ...
  LSTM_NET_FILE = './s2s_vgg_80kvoc_cocolm.words_to_preds.deploy.prototxt'
  ...
  FRAMEFEAT_FILE_PATTERN = '/scratch/cluster/vsub/youtube_dataset/splits/yt_allframes_vgg_fc7_{0}.txt'
  SENTS_FILE_PATTERN = '/scratch/cluster/vsub/youtube_dataset/splits/sents_{0}_lc_nopunc.txt'
  ...
  vocab_file = '%s/vocabs/yt_coco_surface_80k_vocab.txt' % LSTM_DIR
  ...
  for split_name, data_split_name, aligned in DATASETS:
    ...
    NUM_CHUNKS = (len(fsg.vid_framefeats)/NUM_OUT_PER_CHUNK) + 1 # num videos in batches of 30
    eos_string = '<EOS>'
